package com.bookLoansystem.demo.Service.StudentService;

import com.bookLoansystem.demo.DTOs.StudentDto;
import com.bookLoansystem.demo.Entity.Course;
import com.bookLoansystem.demo.Entity.Student;
import com.bookLoansystem.demo.Exception.ResourceNotFoundException;
import com.bookLoansystem.demo.Mapper.Mapper;
import com.bookLoansystem.demo.Repositories.CourseRepository;
import com.bookLoansystem.demo.Repositories.StudentRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Service;

@Service
public class DropACourseService {

    @Autowired
    private CourseRepository courseRepository;
    @Autowired
    private StudentRepository studentRepository;
    @Autowired
    private Mapper mapper;

    public ResponseEntity<StudentDto> dropCourse(Long studentId , Long courseId){

        Student student = studentRepository
                .findById(studentId).orElseThrow(() -> new ResourceNotFoundException("cant find Student by ID: " + studentId));

        Course course = courseRepository
                .findById(courseId).orElseThrow(() -> new ResourceNotFoundException("cant find course by ID: " +courseId));

        if (!student.getCourses().contains(course)){
            throw new RuntimeException("student is not enrolled to this course");
        }

        student.getCourses().remove(course);
        studentRepository.save(student);
        var result = mapper.toStudentDto(student);
        return ResponseEntity.status(HttpStatus.OK).body(result);

    }

}
